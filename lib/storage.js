...
(function () {
  ...
  // 规则：置顶区在前，非置顶区在后。
  //   置顶区内：越晚置顶越靠前（pinnedAt 降序）—— 新置顶的"跳"到最顶。
  //   非置顶区：保持传入顺序（即用户拖拽后的顺序），不做重排。
  function sortSites(sites) {
    if (!Array.isArray(sites)) return [];
    const pinned = sites
      .filter((s) => s && s.pinned)
      .slice()
      .sort((a, b) => (b.pinnedAt || 0) - (a.pinnedAt || 0)); // 降序：晚置顶在前
    const others = sites.filter((s) => s && !s.pinned);
    return pinned.concat(others);
  }

  // 切换某站点的置顶态。返回新数组（不改原数组）。
  //   置顶：pinned=true, pinnedAt=Date.now()
  //   取消：pinned=false, pinnedAt=null
