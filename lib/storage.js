...
(function () {
  ...
  // ---------- 站点排序与置顶（纯函数，供侧边栏/设置页/悬浮窗共用） ----------
  // 规则：仅按区域分组，不改变组内顺序。
  //   - 置顶区（pinned=true）恒在数组前段
  //   - 非置顶区恒在后段
  //   - 两组各自保留传入顺序（即用户拖拽后的顺序）
  //   设计意图：顺序完全交给手动拖拽，本函数只保证"置顶在前"这一条不变式，
  //   因此拖拽改顺序后不会被任何时间规则覆盖。
  function sortSites(sites) {
    if (!Array.isArray(sites)) return [];
    const pinned = sites.filter((s) => s && s.pinned);
    const others = sites.filter((s) => s && !s.pinned);
    return pinned.concat(others);
  }

  // 切换某站点的置顶态。返回新数组（不改原数组）。
  //   置顶：pinned=true，并把该站点移到「置顶区末尾」
  //   取消：pinned=false，并把该站点移到「非置顶区末尾」
  //   不使用 pinnedAt —— 区域内顺序完全由拖拽决定。
  function togglePin(sites, id) {
    if (!Array.isArray(sites)) return [];
    const target = sites.find((s) => s.id === id);
    if (!target) return sites.slice();
    const nextPinned = !target.pinned;
    const updated = Object.assign({}, target, { pinned: nextPinned });
    // 移除原项，按"放到对应区末尾"重新拼接
    const rest = sites.filter((s) => s.id !== id);
    const restPinned = rest.filter((s) => s && s.pinned);
    const restOthers = rest.filter((s) => s && !s.pinned);
    if (nextPinned) {
      return restPinned.concat([updated], restOthers); // 放置顶区末尾
    }
    return restPinned.concat(restOthers, [updated]); // 放非置顶区末尾
  }
